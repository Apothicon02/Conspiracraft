...
public class Buffer {
    ...
    public Buffer(MemoryStack stack, int bufferSize, int usage, int properties, boolean temporary) {
        ...
        this.memory = new long[1];
        this.usage = usage;
        this.pointer = PointerBuffer.allocateDirect(1);
        ...
        if (!temporary) {
            ...
            createDescriptor(stack, this);
        }
    }

    public static void recreateDescriptors(MemoryStack stack) {
        for (Buffer buf : buffers) {createDescriptor(stack, buf);}
    }
    public static void createDescriptor(MemoryStack stack, Buffer buf) {
        if ((buf.usage & VK_BUFFER_USAGE_STORAGE_BUFFER_BIT) != 0) {
