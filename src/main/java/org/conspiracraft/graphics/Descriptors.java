...
import org.conspiracraft.renderer.buffers.ShaderStorageBuffer;
...
public class Descriptors {
    ...
    public void createDescriptorPool(MemoryStack stack) {
        ...
        int b = 0;
        for (int i = 0; i < UniformBuffer.buffers.size(); i++) {
            poolSizes.get(b)
    ...
    public void createDescriptorSetLayout(MemoryStack stack) {
        ...
        for (int i = 0; i < UniformBuffer.buffers.size(); i++) {
            layoutBindings.get(b)
                    ...
                    .descriptorType(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
                    .descriptorCount(1)
                    .stageFlags(UniformBuffer.buffers.get(i).stageFlags);
            b++;
        }
        for (int i = 0; i < ShaderStorageBuffer.buffers.size(); i++) {
            layoutBindings.get(b)
        ...
        Graphics.createGraphicsPipeline(stack);
    }
