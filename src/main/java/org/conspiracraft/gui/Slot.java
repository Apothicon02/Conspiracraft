            ...
            if (GUI.canInteract) {
                ...
                if (Main.player.inputHandler.leftButtonPressed) {
                    ...
                    if (slotReal) {
                        if (Main.timeMsLong - prevLClick < 200) { //double click
                            if (Main.player.inv.cursorItem == null) { //pickup item hovering over if none is being carried by cursor
                                Main.player.inv.cursorItem = menu.items[id].clone();
                                menu.items[id] = null;
                            }
                            if (Main.player.inv.cursorItem != null) {
                                ...
                                for (Pair<Integer, Item> pair : sortedItems) {
                                    ...
                                    if (item.amount <= 0) {
                                        menu.items[pair.component1()] = null;
                                    }
                                }
                            }
                        } else if (Main.player.inv.cursorItem == null) {
                            if (menu.items[id] != null) {
                                Main.player.inv.cursorItem = menu.items[id].clone();
                                menu.items[id] = null;
                            }
                        } else if (menu.items[id] == null || menu.items[id].amount <= 0) {
                            menu.items[id] = Main.player.inv.cursorItem.clone();
                            Main.player.inv.cursorItem = null;
                        } else if (menu.items[id].type == Main.player.inv.cursorItem.type) {
                            int transferAmt = Math.min(Main.player.inv.cursorItem.amount, menu.items[id].type.maxStackSize - menu.items[id].amount);
                            Main.player.inv.cursorItem.amount -= transferAmt;
                            menu.items[id].amount += transferAmt;
                            if (Main.player.inv.cursorItem.amount <= 0) {
                                Main.player.inv.cursorItem = null;
                            }
                        } else {
                            Item newCursorItem = menu.items[id].clone();
                            menu.items[id] = Main.player.inv.cursorItem.clone();
                    ...
                    if (slotReal) {
                        if (Main.player.inv.cursorItem == null) {
                            if (menu.items[id] != null) {
                                int transferAmt = menu.items[id].amount / 2;
                                Main.player.inv.cursorItem = menu.items[id].clone();
                                Main.player.inv.cursorItem.amount = transferAmt;
                                menu.items[id].amount -= transferAmt;
                                if (menu.items[id].amount <= 0) {
                                    menu.items[id] = null;
                                }
                            }
                        } else if (menu.items[id] == null || menu.items[id].amount <= 0) {
                            menu.items[id] = Main.player.inv.cursorItem.clone();
                            menu.items[id].amount = 1;
                            Main.player.inv.cursorItem.amount -= 1;
                            if (Main.player.inv.cursorItem.amount <= 0) {
                                Main.player.inv.cursorItem = null;
                            }
                        } else if (menu.items[id].type == Main.player.inv.cursorItem.type) {
                            int transferAmt = Math.min(1, menu.items[id].type.maxStackSize - menu.items[id].amount);
                            Main.player.inv.cursorItem.amount -= transferAmt;
                            menu.items[id].amount += transferAmt;
                            if (Main.player.inv.cursorItem.amount <= 0) {
                                Main.player.inv.cursorItem = null;
